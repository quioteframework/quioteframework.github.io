...
For the full mechanism and design behind these steps, see [Plugins and extensibility](/architecture/plugins/) and [The middleware pipeline](/architecture/middleware-pipeline/) / [Writing custom middleware](/advanced/custom-middleware/).
...
<Aside type="note" title="How your plugin fits in a request">
`register()` runs **once at boot**, not per request: `PluginManager` reads `Config/plugins.*`, checks the `#[Plugin]` attribute, and calls `register()` in declared order — after your `settings.*` load, before any request is served. Whatever you contributed is then simply present when requests run. The `HealthzMiddleware` above joins the pipeline as a station (`ErrorHandlingMiddleware` → … → `RoutingMiddleware` → **`HealthzMiddleware`** → … → `DispatchMiddleware`), placed by its `#[Middleware(phase: 'pre_routing', …)]`; a `configDefault` or `service` instead becomes a value an action or middleware reads mid-request. See [Plugins overview: How a plugin reaches a request](/plugins/overview/#how-a-plugin-reaches-a-request).
</Aside>
