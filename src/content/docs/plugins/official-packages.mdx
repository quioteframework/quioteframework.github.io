...
OIDC login (`OidcClient`, `OidcAuthenticator` for the callback leg, `OidcStateStorage`) and machine-to-machine tokens (`ClientCredentialsClient`, `IntrospectionClient`), also built on `quioteframework/auth`'s contracts and reusing `auth-jwt`'s `TokenValidatorInterface` for ID token validation rather than a second JWT stack. PKCE S256 is hardcoded (OAuth 2.1 mandates it). No plugin ships with this package — every piece needs app-specific secrets or endpoints, so there's nothing safe to register by default.

```bash
composer require quioteframework/auth-oauth
```

See [`quioteframework/auth-oauth` — OIDC client and machine-to-machine](/advanced/authentication-authorization/#quioteframeworkauth-oauth--oidc-client-and-machine-to-machine).
