...
| Propulsion | Propulsion runtime service container |
...
:::caution[Forthcoming]
Propulsion is a PHP 8.5 rewrite of Propel; its adapter ships after the others. The alias, parameters, and accessors below follow Propel's runtime model and the conventions of the adapters above — treat them as provisional until the adapter lands.
:::

`class="propulsion"` → `PropulsionDatabase`. Requires the Propulsion runtime + `PropulsionPlugin`. `getConnection()` returns the Propulsion runtime service container.

Like classic Propel, Propulsion is **code-generated**: you compile model and query classes from a schema ahead of time (a build/console concern) and autoload them via Composer; the adapter wires the runtime connection those classes use. Supply generated, cached classes rather than regenerating on every boot — the same posture as Cycle's schema.

| Parameter | Default | Description |
|---|---|---|
| `connection` | — | Name of another database to borrow a PDO from (layer mode), or an inline params array. |
| `adapter` | — | `mysql` \| `pgsql` \| `sqlite` \| `sqlsrv` \| `oracle` (required unless via `connection`) |
| `dsn` | — | PDO DSN |
| `user` / `username`, `password` | — | Credentials |
| `datasource` | `default` | Datasource name the generated classes bind to |
| `settings` | `[]` | Runtime connection settings (charset, attributes, on-connect queries) |
...
<database name="main" class="propulsion">
  <ae:parameter name="adapter">pgsql</ae:parameter>
  <ae:parameter name="dsn">pgsql:host=localhost;dbname=app</ae:parameter>
  <ae:parameter name="user">app</ae:parameter>
  <ae:parameter name="password">secret</ae:parameter>
</database>
...
$sc   = $db->getServiceContainer();     // Propulsion runtime service container
$conn = $db->getPropelConnection();     // ConnectionInterface (wraps the PDO)

// Generated query + model classes (autoloaded via Composer):
$books = \App\Model\BookQuery::create()->filterByPublished(true)->find();
```

Propel-style ORMs keep an **instance pool** per model; the adapter clears it at the worker request boundary so identity state never leaks between requests, while the connection is kept and recycled like every other adapter.
...
| Propulsion | ✅ borrows the referenced PDO (`adapter` still required) |
...
The ORM libraries are optional (`suggest`, not `require`). The adapter classes load fine without them and only fail — with an actionable message — when you actually connect.
