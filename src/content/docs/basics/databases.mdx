...
|---|---|---|
| Turn the database layer on | `core.use_database` in `Config/settings.{php,yaml,xml}` | [Turning the layer on](#turning-the-layer-on) |
| Your connections (host, credentials, which adapter) | `Config/databases.{php,yaml,xml}` | [Configuring connections](#configuring-connections) |
| The adapter package (so an ORM alias exists) | the `plugins` config, `Config/plugins.{php,yaml,xml}` | [Enabling an ORM adapter](#enabling-an-orm-adapter) |
...
[Eloquent](https://laravel.com/docs/eloquent) is Laravel's active-record ORM and fluent query builder (`illuminate/database`), usable standalone outside a full Laravel application via its **Capsule Manager**. It has by far the largest install base and community of any PHP ORM.
...
- **`getConnection()` returns** the Capsule `Manager` — *not* the underlying connection. A typed accessor, `getEloquentConnection()`, reaches the connection itself (see below). Returning the Capsule is what lets the framework's worker-mode `ping()`/`reset()`/`shutdown()` machinery manage it uniformly.

:::caution[Eloquent `Model` classes need `global`/`boot_eloquent`]
Eloquent's `Model` classes (as opposed to the plain query builder) resolve their connection through Capsule's *static* connection resolver. Using them requires the `global`/`boot_eloquent` options below — process-global static state set up once at worker boot, not something to redo per request.
:::
...
[DBAL](https://www.doctrine-project.org/projects/dbal.html) is Doctrine's connection abstraction and SQL query builder **without** the entity/unit-of-work layer above it. Reach for it when you want vendor-portable SQL and a fluent query builder but don't want to commit to entity mapping — reporting queries, admin tooling, or a gradual migration path onto (or off of) the full ORM.
...
[Cycle](https://cycle-orm.dev/) (v2) comes out of the Spiral/RoadRunner ecosystem — a data-mapper ORM like Doctrine, but purpose-built to be stateless and schema-first so it behaves correctly across thousands of requests in one long-lived process, rather than assuming PHP's traditional request-per-process model.
...
- **Requires** the [`quioteframework/propulsion`](https://github.com/quioteframework/propulsion) runtime + `quioteframework/db-propulsion`'s `PropulsionPlugin`
