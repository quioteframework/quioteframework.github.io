...
`DispatchMiddleware` runs the whole read-cache / execute-on-miss / store cycle. On a hit it replays the stored view result without re-running the action; on a miss it executes, then stores with your TTL.
