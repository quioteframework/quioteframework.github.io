...
The scaffolded app uses `NullStorage`.

:::danger[NullStorage means no session cookie, which means no working CSRF protection]
With `NullStorage`, no session is ever persisted and no session cookie is ever sent — for the entire app, on every request. `CsrfValidationMiddleware` treats **any** request with no session cookie as exempt from CSRF checking (no ambient credential, nothing for an attacker to ride — see [Authentication & authorization: CSRF protection](/advanced/authentication-authorization/#automatic-exemptions--and-the-nullstorage-trap)). With `NullStorage` active, that exemption fires on *every* request, so CSRF validation never actually runs, even though the `_csrf_token` hidden field is still injected into every non-GET form and looks like it's doing something.

If your app has any state-changing form or endpoint a browser session could be tricked into submitting, switch to `SessionStorage` or the PDO backend below **before** relying on CSRF protection. `NullStorage` is only correct for genuinely stateless apps/APIs, where there's no ambient session cookie for CSRF to protect in the first place.
:::

Switch to real sessions by changing the `storage` factory:
...
'storage' => [
    ...
    'params' => [
        ...
        'database' => 'default',   // connection name; omit to use the app's default connection
    ],
],
```
...
You never start or close the session yourself — `SessionMiddleware` does it. The kernel discovers it the same way as every other middleware (the `MiddlewareAttributeScanner` scans `#[Middleware]` attributes, the `MiddlewareOrderResolver` orders them); `SessionMiddleware` declares the `bootstrap` phase with a high priority, which places it early — before security — so storage is live before anything that depends on a user. Its path through a request:

> Kernel boots and builds the pipeline → request enters → `SessionMiddleware` mirrors the incoming cookie into place and starts storage → `SecurityMiddleware` reads/writes the authenticated user via storage → the action runs → on the way out `SessionMiddleware` writes and closes the session, then bridges any queued cookies onto the response.
...
On the way in `SessionMiddleware` starts the session (mirroring the incoming cookie into place first); on the way out it writes and closes it. You rarely touch this directly — the important method for privilege transitions is regeneration:

```php
$storage->regenerate();   // new session id, keeps the data — call on login
```

Regenerating the id on the unauthenticated-to-authenticated transition defeats session fixation. `SecurityUser::setAuthenticated(true)` does this for you (see the [auth page](/advanced/authentication-authorization/#securityuser)), so you usually get it for free.
