...
1. `RoutingMiddleware` resolves the request to a `_module` and `_action` (see [Routing](/basics/routing/)).
...
> `RoutingMiddleware` resolves `_module` / `_action` → `DispatchMiddleware` asks the `Controller` to instantiate the action class in that module → the action runs → `ViewNameResolver` locates the view and template in the same module → the response is rendered.
