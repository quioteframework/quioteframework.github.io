...
Because parameter access is whitelist-enforced under strict validation, a route parameter you read should be covered by a validator or accessed with a default. See [Validation](/basics/validation/).
...
return [
    ...
    'routing.http_method_map' => [
        ...
        'LOCK'  => 'lock',    // add a non-standard verb → executeLock()
    ],
];
```
...
- The token becomes the method name, `ucfirst`-ed: token `lock` → `executeLock()`. Any action meant to handle that verb needs the matching method.
- The token also names the per-method validation hooks — token `lock` → `registerLockValidators()`, `validateLock()`, `handleLockError()` (see [Validation](/basics/validation/)).
