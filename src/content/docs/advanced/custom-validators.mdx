...
This page is for that case. [Validation](/basics/validation/) covers using the built-ins and [Advanced validation](/advanced/advanced-validation/) shows the quick way to slot a custom class in with `raw()`. Here we go a level down: the `Validator` base class contract, the `validators.xml` file format, and how validators are discovered, compiled, and run.

:::note[How a validator fits in a request]
Validators don't run when you construct them — they run as part of the request pipeline, just before your action. In short:

> `RoutingMiddleware` resolves the action → `SecurityMiddleware` checks access → **`ValidationMiddleware`** compiles the action's validators, runs them against the request, and prunes every parameter that no validator claimed → `DispatchMiddleware` runs the action, which now reads only validated (and optionally sanitized) input.

So a validator does two jobs at once: it *checks* a field, and — because of that pruning — it is what *makes the field readable* to the action at all. See [The middleware pipeline](/architecture/middleware-pipeline/) and [strict parameter access](/basics/requests-and-responses/#strict-parameter-access).
:::
