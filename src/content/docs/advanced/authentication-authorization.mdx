...
CSRF is on by default (`core.csrf.enabled`), enforced by two middlewares (see [The middleware pipeline](/architecture/middleware-pipeline/)):
...
The `Quiote\Security\RateLimit` components provide login throttling (`LoginThrottle`) with pluggable storage (e.g. `PdoRateLimiterStorage`), so repeated failed logins can be slowed or blocked. Wire it into your login action to defend against credential-stuffing.
