...
Telemetry lives outside the kernel, in the optional **[`quioteframework/telemetry-otel`](/plugins/official-packages/#quioteframeworktelemetry-otel)** package (it carries the `open-telemetry/*` SDK). A bare install pulls none of it, and every OTel call site is guarded — the always-on `Trace` facade stays in the core as a no-op — so its absence is a fully supported state. To use telemetry, `composer require quioteframework/telemetry-otel` (and, for the OTLP exporter, a PSR-18 HTTP client). For local inspection, the companion **[`quioteframework/telemetry-dashboard`](/plugins/official-packages/#quioteframeworktelemetry-dashboard)** package adds a `telemetry:dashboard` terminal UI that receives OTLP directly.
:::
...
> Request enters `ErrorHandlingMiddleware` → **`TelemetryMiddleware`** opens the root `Quiote.Http` server span and enriches `LogContext` with `trace_id`/`span_id` → `RoutingMiddleware` renames the span to the route identity → the action and view open nested spans → response returns → the middleware records metrics and closes the span → telemetry is flushed at the worker reset.

When telemetry is off the middleware is a single `if (!Trace::enabled())` pass-through, so leaving it in the pipeline is free. See [The middleware pipeline](/architecture/middleware-pipeline/).
