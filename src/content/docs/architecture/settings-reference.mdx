...
| `core.developer_exceptions` | `false` | `true` → detailed Whoops error page; `false` → safe generic response. Independent of `core.debug`. See [ErrorHandlingMiddleware](/architecture/middleware-reference/#errorhandlingmiddleware). |
...
| `core.correlation_id.header` | `'X-Correlation-Id'` | Inbound header adopted as the request's correlation id (sanitized, length-capped); absent → one is generated. Echoed back on the response. See [Logging → Correlation IDs](/architecture/logging/#correlation-ids). |
...
| Key | Default | Effect |
|---|---|---|
| `routing.http_method_map` | `[]` | Overrides the HTTP-verb → action-method map. See [Routing](/basics/routing/#customising-the-http-verb-mapping). |
...
| `plugins` | `[]` | List of plugin class-strings to register at boot (a **bare** key — in XML use `prefix=""`). See [Plugins and extensibility](/architecture/plugins/). |
...
| Name | Default | Effect |
|---|---|---|
| `QUIOTE_ENV` | `'prod'` | Environment name → `core.environment`. Note: the scaffolded `pub/index.php` passes `'development'` explicitly, so the effective default depends on your front controller. |
