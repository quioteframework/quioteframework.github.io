...
If anything downstream throws, the stack unwinds back up to `ErrorHandlingMiddleware`'s single `try/catch`, which turns the throwable into a response. For the whole pipeline see [The middleware pipeline](/architecture/middleware-pipeline/) and [The request lifecycle](/architecture/request-lifecycle/).

## The flow

Because it sits at the very top of the [pipeline](/architecture/middleware-pipeline/) (phase `bootstrap`, highest priority), `ErrorHandlingMiddleware` is the last thing to see the request on the way in and the first to catch anything on the way out. When a `Throwable` escapes the stack it:
