...
- **`true` → `WhoopsRenderer`.** The full [Whoops](https://github.com/filp/whoops) developer page — stack trace, source, request data — or its JSON/plain-text handlers depending on `Accept`. It is configured to return markup as a string rather than writing to output, so it is safe under [worker mode](/architecture/deployment/).
