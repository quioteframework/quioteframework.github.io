...
description: The generated Active Record method API — getters, setters, persistence, relationships, and lifecycle hooks.
---

Propulsion generates Active Record classes based on the schema definition of tables. Active Record objects offer a powerful API to manipulate database records in an intuitive way. This API is essentially unchanged from Propel 1 — only the class/namespace names have been renamed (`Propel*` → `Propulsion*`); see [Migrating from Propel 1](/propulsion/getting-started/migrating-from-propel/) for the full list of renames.
...
// ActiveRecordInterface is bundled with Propulsion
...
See [Relationships](/propulsion/basics/relationships/) for more details.
...
:::tip
By default, Propulsion uses the name of the Active Record class to generate the related object getter and setter. However, you can customize this name by setting the `phpName` attribute in the `<foreign-key>` tag:
:::
...
:::tip
CLOB (_Character_ Locator Objects) are treated as strings in Propulsion, so the getter and setter for a CLOB column have no special functionality.
:::
...
:::tip
OBJECT columns are searchable, given an object as the search value. See the [Model/Query reference: Column filter methods](/propulsion/reference/model-criteria/#column-filter-methods) for more details.
:::
...
:::tip
ARRAY columns are searchable, given an array or a scalar as the search value. See the [Model/Query reference: Column filter methods](/propulsion/reference/model-criteria/#column-filter-methods) for more details.
:::

There is no `UUID_BINARY` column type in Propulsion — see [UUID and binary columns](/propulsion/reference/uuid-binary-columns/) for the current state of UUID storage.
...
:::tip
If you never use these generic getters and setters, you can disable their generation to clean up the Active Record class by setting these build properties (see the [configuration file reference](/propulsion/reference/configuration-file/)):
:::
...
See [Behaviors](/propulsion/behaviors/) for more details.
