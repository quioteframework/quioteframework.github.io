...
Propulsion supports multi-column scoped sortable behavior. Define it with a comma-separated list of column names as the `scope_column` parameter. API methods generated by the behavior take parameters in the order defined by `scope_column`.
...
With this schema, Propulsion manages one sortable list of tasks per user per group, so for each user-group combination:
...
// $t4
$firstJohnUserTask = TaskQuery::create()->findOneByRank(
    rank: 1,
...
```php
$allJohnsUserTasks = TaskQuery::create()
...
bool    reorder($newOrder) // $newOrder is an $id => $rank associative array
// only for behavior with use_scope
array   inList($scope)
```
