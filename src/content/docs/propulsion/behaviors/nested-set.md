...
description: Turn a model into a tree structure using the nested set algorithm.
---
...
:::caution[Known completeness gaps]
Propulsion's nested-set builder family (`NodeBuilder`/`NodePeerBuilder`/`NestedSetBuilder`/`NestedSetPeerBuilder`) carries real completeness gaps inherited from the fork's PHP5-builder removal — some generated methods are literal no-op stubs, and it's unclear this behavior has ever fully worked in this codebase. Read [`KNOWN_ISSUES.md`](https://github.com/quioteframework/propulsion/blob/main/KNOWN_ISSUES.md) before relying on `nested_set` for anything beyond experimentation, and verify the specific methods you need against a real generated model before shipping.
:::

Many applications need to store hierarchical data in the model. For instance, a forum stores a tree of messages for each discussion, a CMS sees sections and subsections as a navigation tree, and in a business organization chart, each person is a leaf of the organization tree. [Nested sets](http://en.wikipedia.org/wiki/Nested_set_model) are a good way to store such hierarchical data in a relational database and manipulate it. The name "nested sets" describes the algorithm used to store the position of a model in the tree; it's also known as "modified preorder tree traversal."
...
If your application used the old nested sets builder from Propel 1.4, you can enable the `method_proxies` parameter so the behavior generates method proxies for methods that used a different name (e.g. `createRoot()` for `makeRoot()`, `retrieveFirstChild()` for `getFirstChild()`, etc.):
