...
description: Keep a column updated with an aggregate function computed over a related table.
---
...
In `schema.xml`, use the `<behavior>` tag to add the `aggregate_column` behavior to a table. You must provide parameters for the aggregate column `name`, the foreign table name, and the aggregate `expression`. For instance, to add an aggregate column keeping the comment count in a `post` table:
...
Rebuild your model, and run the table creation SQL again. The model now has an additional `nb_comments` column, of type `integer` by default. Each time a record in the foreign table is added, modified, or removed, the aggregate column is updated:
...
The aggregate column is also kept up to date when related records are modified through a query object:
...
Any aggregate function can be used on any of the foreign columns. For instance, you can use the `aggregate_column` behavior to keep the latest update date of related comments, or the total votes on comments. You can even keep several aggregate columns on a single table:
...
By default, the behavior adds one column to the model. If this column is already described in the schema, the behavior detects it and doesn't add it a second time. This can be useful if you need a custom `type` or `phpName` for the aggregate column:
