...
- **Bearer auth, safe by default** — set `mcp.auth_token`, typically from an environment variable:
  ```php
  // Config/settings.php
  'mcp.auth_token' => getenv('QUIOTE_ASSISTANT_MCP_TOKEN') ?: null,
  ```
  With no token configured, `Quiote\Mcp\Auth\StaticTokenAuthenticator` **rejects every HTTP request** — there's no accidental "wide open" default. For a trusted network or a reverse proxy that already authenticates, `mcp.auth = 'none'` is the explicit escape hatch.
...
A `Dockerfile` (PHP 8.5-cli-alpine, `intl`/`xsl`/`pdo_sqlite` extensions, `--no-dev` composer install) packages the assistant as an image, published to `ghcr.io/quioteframework/quiote-mcp-assistant` on tagged releases (`:latest` and `:vX.Y.Z`). It ships only `pdo_sqlite` — none of the project-aware tools open a real database connection, so `pdo_mysql`/`pdo_pgsql` aren't needed in the image.
